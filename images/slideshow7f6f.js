(function() {
	...
	// TODO: message about editing newest one!!!
...
function Slideshow(element, options) {
	...
	function renderLink(photo, i) {
		...
		if (isTouch) {
			var a = td.down('a');
			a.href = largeURL(photo);
			a.title =  _.unescape(photo.caption) || '';
		}
		var a = td.select('a')[0];
		if (!isTouch) {
	...
	function addSlide(photo, i) {
		...
		if (photo.alt) {
			// Attribute of alt text must be explicitly set to have escaped string.
			img.alt = photo.alt;
		}
		slides[i] = slide;
		slideImgs[i] = img;
		...
		slide.select('a').each(function(a) {
			if (window.currentSite) {
				...
				a.title = _W.utl('html.weebly.libraries.weebly_slideshow_1');
			}else{
				if (link) {
					a.href = link;
					if (linkNewWindow) {
	...
	function myUp(e, uppercaseTagName) {
		while (e = e.parentNode) {
			if (e.tagName == uppercaseTagName) {
				return e;
			}
		}
	}
	...
	function initLinkSwiping(elm) {
		...
		function touchstart(ev) {
			...
			function touchend(ev) {
				if (simulateClick && elm.nodeName == 'A') {
					// make photoswipe show up
					window.Code.Util.Events.fire(elm, 'click');
				}
				if (p1 === null || p0 === null) {
					linkVelocity = 0;
				}else{
				...
				document
					.stopObserving(TOUCHMOVE, touchmove)
					.stopObserving(TOUCHEND, touchend)
					.stopObserving(TOUCHCANCEL, touchend);
			}

			document
				.observe(TOUCHMOVE, touchmove)
				.observe(TOUCHEND, touchend)
				.observe(TOUCHCANCEL, touchend);

			ev.stop();

		}

		elm.observe(TOUCHSTART, touchstart);
...
function thumbnailURL(photo) {
	var url = photo.url;
	if (!url.match("https://binasbeautysalon.weebly.com/weebly/images/")) {
		url = '/uploads/' + url;
	}
	return url;
}
